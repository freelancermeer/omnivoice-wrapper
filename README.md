...
| **The truncation warning was wrong every time it fired** — 11 of 63 clips flagged, all eleven measuring −91 dB over their final 0.25 s. | It was judged on the last chunk *as the model made it*, which is upstream of the 300 ms of tail padding the same function appends. It now measures the audio the caller actually receives; a clip cut off mid-word still warns, a padded one never does. |
...
| **One request in eleven returned nothing at all** — accepted, worked 26-41 s, then closed the connection with no status, no body and every counter at zero. Deterministic: the same three segments of an eleven-segment video, every run, on every voice. | Not a crash. The normalized script is echoed in a header truncated to 480 characters, and when character 479 lands on a space h11 refuses to send it — inside `send()`, after the route returned, where FastAPI can no longer make it a 500. `_hdr()` now strips **after** truncating, `_safe_headers()` repairs anything unsendable before the response leaves, and `tests/test_headers.py` checks every truncation offset. |
